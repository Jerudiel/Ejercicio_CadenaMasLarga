...
#include "qroundprogressbaralarm.h"

#include <QtGui/QPainter>


QRoundProgressBarAlarm::QRoundProgressBarAlarm(QWidget *parent) :
    QWidget(parent),
    m_min(0), m_max(100),
    m_value(25),
    m_nullPosition(PositionTop),
    m_barStyle(StyleLine),
    m_outlinePenWidth(1),
    m_dataPenWidth(2),
...
{
    try {
        m_background_color_activo = new QColor(255,112,61);
        m_background_color_inactivo = new QColor(255,255,255);
        m_background_color = new QColor(255,255,255);
        textcolor = new QColor(0,0,0);
    }  catch (std::exception &e) {
        qWarning("Error %s desde la funcion %s", e.what(), Q_FUNC_INFO );
    }
}

void QRoundProgressBarAlarm::setRange(double min, double max)
{
    try {
        m_min = min;
        m_max = max;
...
void QRoundProgressBarAlarm::setMinimum(double min)
{
    try {
         setRange(min, m_max);
    }  catch (std::exception &e) {
...
void QRoundProgressBarAlarm::setMaximum(double max)
{
    try {
         setRange(m_min, max);
    }  catch (std::exception &e) {
...
void QRoundProgressBarAlarm::setValue(double val)
{
    try {
        if (m_value != val)
        {
...
void QRoundProgressBarAlarm::setValue(int val)
{
    try {
         setValue(double(val));
    }  catch (std::exception &e) {
...
void QRoundProgressBarAlarm::setNullPosition(double position)
{
    try {
...
void QRoundProgressBarAlarm::setBarStyle(QRoundProgressBarAlarm::BarStyle style)
{
    try {
        if (style != m_barStyle)
        {
...
void QRoundProgressBarAlarm::setOutlinePenWidth(double penWidth)
{
    try {
...
void QRoundProgressBarAlarm::setDataPenWidth(double penWidth)
{
    try {
...
void QRoundProgressBarAlarm::setDataColors(const QGradientStops &stopPoints)
{
    try {
...
void QRoundProgressBarAlarm::setFormat(const QString &format)
{
    try {
        if (format != m_format)
        {
...
void QRoundProgressBarAlarm::resetFormat()
{
    try {
...
void QRoundProgressBarAlarm::setDecimals(int count)
{
    try {
...
void QRoundProgressBarAlarm::setUnit(QString unit)
{
    try {
         m_unit = unit;
    }  catch (std::exception &e) {
...
void QRoundProgressBarAlarm::setInactive()
{
    try {
         m_background_color = m_background_color_inactivo;
         update();
    }  catch (std::exception &e) {
        qWarning("Error %s desde la funcion %s", e.what(), Q_FUNC_INFO );
    }
...
void QRoundProgressBarAlarm::setActive()
{
    try {
         m_background_color = m_background_color_activo;
         update();
    }  catch (std::exception &e) {
        qWarning("Error %s desde la funcion %s", e.what(), Q_FUNC_INFO );
    }
...
void QRoundProgressBarAlarm::paintEvent(QPaintEvent* /*event*/)
{
    try {
...
void QRoundProgressBarAlarm::drawBackground(QPainter &p, const QRectF &baseRect)
{
    try {
...
void QRoundProgressBarAlarm::drawBase(QPainter &p, const QRectF &baseRect)
{
    try {
        switch (m_barStyle)
        {
...
void QRoundProgressBarAlarm::drawValue(QPainter &p, const QRectF &baseRect, double value, double arcLength)
{
    try {
        // nothing to draw
        if (value == m_min)
...
void QRoundProgressBarAlarm::calculateInnerRect(const QRectF &/*baseRect*/, double outerRadius, QRectF &innerRect, double &innerRadius)
{
    try {
        // for Line style
        if (m_barStyle == StyleLine)
...
void QRoundProgressBarAlarm::drawInnerBackground(QPainter &p, const QRectF &innerRect)
{
    try {
        if (m_barStyle == StyleDonut)
        {
            //p.setBrush(palette().alternateBase());
            p.setBrush(QBrush(*m_background_color));
            p.drawEllipse(innerRect);
        }
        else if( m_barStyle == StyleLine){
            p.setBrush(QBrush(*m_background_color));
            p.setPen(QPen(*m_background_color, 1)); //(m_background_color,1)
            p.drawEllipse(innerRect);
        }
    }  catch (std::exception &e) {
        qWarning("Error %s desde la funcion %s", e.what(), Q_FUNC_INFO );
...
void QRoundProgressBarAlarm::drawText(QPainter &p, const QRectF &innerRect, double innerRadius, double value)
{
    try {
        if (m_format.isEmpty())
            return;
        ...
        //int maxWidth = fm->horizontalAdvance(valueToText(m_max));
        //float delta = innerRadius / maxWidth;
        //int fontSize = f.pixelSize() * delta * 12;
        //f.setPixelSize(fontSize);
        //
        //p.setFont(f);

        QRectF textRect(innerRect);
        p.setPen(*textcolor); //palette().text().color())
...
QString QRoundProgressBarAlarm::valueToText(double value) const
{
    try {
        ...
        {
            textToDraw.replace("%d", QString::number(value, 'f', m_decimals));
            textToDraw = textToDraw + "\n" + m_unit;
        }
...
void QRoundProgressBarAlarm::valueFormatChanged()
{
    try {
        m_updateFlags = 0;
...
void QRoundProgressBarAlarm::rebuildDataBrushIfNeeded()
{
    try {
        if (m_rebuildBrush)
        {
