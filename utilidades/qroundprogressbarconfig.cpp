...
#include "qroundprogressbarconfig.h"

#include <QtGui/QPainter>


QRoundProgressBarConfig::QRoundProgressBarConfig(QWidget *parent) :
    QWidget(parent),
    m_min(0), m_max(100),
    m_value(25),
...
void QRoundProgressBarConfig::setRange(double min, double max)
{
    try {
        m_min = min;
        m_max = max;
...
void QRoundProgressBarConfig::setMinimum(double min)
{
    try {
         setRange(min, m_max);
    }  catch (std::exception &e) {
...
void QRoundProgressBarConfig::setMaximum(double max)
{
    try {
         setRange(m_min, max);
    }  catch (std::exception &e) {
...
void QRoundProgressBarConfig::setValue(double val)
{
    try {
        if (m_value != val)
        {
...
void QRoundProgressBarConfig::setValue(int val)
{
    try {
         setValue(double(val));
    }  catch (std::exception &e) {
...
void QRoundProgressBarConfig::setNullPosition(double position)
{
    try {
...
void QRoundProgressBarConfig::setBarStyle(QRoundProgressBarConfig::BarStyle style)
{
    try {
        if (style != m_barStyle)
        {
            m_barStyle = style;
            m_rebuildBrush = true; //jeru
            update();
        }
    }  catch (std::exception &e) {
        qWarning("Error %s desde la funcion %s", e.what(), Q_FUNC_INFO );
...
void QRoundProgressBarConfig::setOutlinePenWidth(double penWidth)
{
    try {
...
void QRoundProgressBarConfig::setDataPenWidth(double penWidth)
{
    try {
...
void QRoundProgressBarConfig::setDataColors(const QGradientStops &stopPoints)
{
    try {
...
void QRoundProgressBarConfig::setFormat(const QString &format)
{
    try {
        if (format != m_format)
        {
...
void QRoundProgressBarConfig::resetFormat()
{
    try {
...
void QRoundProgressBarConfig::setDecimals(int count)
{
    try {
...
void QRoundProgressBarConfig::setUnit(QString unit)
{
    try {
         m_unit = unit;
    }  catch (std::exception &e) {
...
void QRoundProgressBarConfig::paintEvent(QPaintEvent* /*event*/)
{
    try {
        ...
        double delta = 0;
        if(m_value > 0){
            delta = (m_max - m_min) / (m_value - m_min);
        }
        else{
            delta=0;
        }
...
void QRoundProgressBarConfig::drawBackground(QPainter &p, const QRectF &baseRect)
{
    try {
...
void QRoundProgressBarConfig::drawBase(QPainter &p, const QRectF &baseRect)
{
    try {
        switch (m_barStyle)
        {
...
void QRoundProgressBarConfig::drawValue(QPainter &p, const QRectF &baseRect, double value, double arcLength)
{
    try {
        // nothing to draw
        if (value == m_min)
        ...
        if(value == m_max){
            ...
            dataPath.arcTo(baseRect, m_nullPosition, -delta);
...
///*baseRect*/
void QRoundProgressBarConfig::calculateInnerRect(const QRectF &/*baseRect*/, double outerRadius, QRectF &innerRect, double &innerRadius)
{
    try {
        // for Line style
        if (m_barStyle == StyleLine or m_barStyle == StyleExpand)
        {
...
void QRoundProgressBarConfig::drawInnerBackground(QPainter &p, const QRectF &innerRect)
{
    try {
...
void QRoundProgressBarConfig::drawText(QPainter &p, const QRectF &innerRect, double innerRadius, double value)
{
    try {
        if (m_format.isEmpty())
            return;
...
QString QRoundProgressBarConfig::valueToText(double value) const
{
    try {
        ...
        {
            textToDraw.replace("%d", QString::number(value));
            textToDraw = textToDraw + "\n" + m_unit;
        }
...
void QRoundProgressBarConfig::valueFormatChanged()
{
    try {
        m_updateFlags = 0;
...
void QRoundProgressBarConfig::rebuildDataBrushIfNeeded()
{
    try {
        if (m_rebuildBrush)
        {
