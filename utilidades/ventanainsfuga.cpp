#include "ventanainsfuga.h"

VentanaInsFuga::VentanaInsFuga(QWidget *parent, int width, int height, QString titulo) : QWidget(parent)
{
    try {
        ...
        label_info->setText("Conectar el circuito del paciente \n y el pulmón artificial");
        ...
        for(int i = 0 ; i < lista.size(); i++){
            ...
            imagenes[i] = new QPixmap(ruta + "/graficos/fuga/" + lista.at(i));
...
void VentanaInsFuga::cambiaImagen(){
    try {
...
void VentanaInsFuga::setTituloColor(QColor color){
    try {
        titulo_color = color;
    } catch (std::exception &e) {
        qWarning("Error %s desde la funcion %s", e.what(), Q_FUNC_INFO );
...
void VentanaInsFuga::setMensajeColor(QColor color){
    try {
        mensaje_color = color;
    }  catch (std::exception &e) {
        qWarning("Error %s desde la funcion %s", e.what(), Q_FUNC_INFO );
...
void VentanaInsFuga::setFondoColor(QColor color){
    try {
...
void VentanaInsFuga::paintEvent(QPaintEvent* /*event*/){
    try {
       QSize s = size();
...
QColor VentanaInsFuga::colorEstado(QString estado){
    try {
        if(estado == PROCESO){
