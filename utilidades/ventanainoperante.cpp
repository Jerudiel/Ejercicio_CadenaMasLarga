#include "ventanainoperante.h"

VentanaInoperante::VentanaInoperante(QWidget *parent, int width, int height, QString titulo) : QWidget(parent)
{
    ...
    label_info->setGeometry(QRect(offset_x + 10, offset_y + 5, 400, 175));
    label_info->setFont(*fuente);
    label_info->setStyleSheet("color: black; background-color: #D5D8DC;"); //#D5D8DC
    label_info->setAlignment(Qt::AlignLeft);
    label_info->setText("FALLA EN TARJETA \n \n No es posible iniciar o actualizar \n el ventilador.");
    ...
    btn_ok->setGeometry(QRect(offset_x + 175, offset_y + 165, 150, 50));

}

void VentanaInoperante::mostrar(QString mensaje){
    //label_info->setText("FALLA EN TARJETA \n \n No es posible iniciar o actualizar \n el ventilador.");
    label_info->setText(mensaje);
    show();
}

void VentanaInoperante::setTitulo(QString nuevo_titulo){
    titulo = nuevo_titulo;
}


void VentanaInoperante::setTituloColor(QColor color){
    try {
        titulo_color = color;
...
void VentanaInoperante::setMensajeColor(QColor color){
    try {
        mensaje_color = color;
    }  catch (std::exception &e) {
        qWarning("Error %s desde la funcion %s", e.what(), Q_FUNC_INFO );
...
void VentanaInoperante::setFondoColor(QColor color){
    try {
...
void VentanaInoperante::paintEvent(QPaintEvent* /*event*/){
    try {
       QSize s = size();
...
QColor VentanaInoperante::colorEstado(QString estado){
    try {
        if(estado == PROCESO){
