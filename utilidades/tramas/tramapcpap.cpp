#include "tramapcpap.h"

TramaPCPAP::TramaPCPAP(Formulas *formulas)
{
    try {
        cpap = "";
        ps = "";
...
void TramaPCPAP::llenar(QStringList parametros, QString alarpmax){
    try {
        ...
        pinsp = parametros.at(9);
        tinsp = parametros.at(10);
        palarmax = alarpmax;
    }  catch (std::exception &e) {
        qWarning("Error %s desde la funcion %s", e.what(), Q_FUNC_INFO );
    }
...
QString TramaPCPAP::obtener_trama_serpresion_config(){
    try {
        QString modo = "4";
        QString t_cpap = obtener_cpap();
        QString t_ps = obtener_ps();
        QString t_fio2 = obtener_fio2();
        QString t_modo_trigger = obtener_modo_trigger();
        ...
        //unir
        QString trama = "C" + modo + t_cpap + t_ps + t_fio2 + t_modo_trigger + t_sensibilidad + t_apnea + t_frecuencia + t_pinsp + t_tinsp + "\n";
        if(trama.size() == 22){
...
QString TramaPCPAP::obtener_cpap(){
    try {
        QString cpap_s = "";
...
QString TramaPCPAP::obtener_fio2(){
    try {
        QString oxi_s = "";
...
QString TramaPCPAP::obtener_modo_trigger(){
    try {
        if(mod_trigger == "1"){
            return "1";
        }
...
QString TramaPCPAP::obtener_sensibilidad(){
    try {
        QString sensi_s = "";
        QString temp_s = "";
        if(mod_trigger == "1"){
...
QString TramaPCPAP::obtener_apnea(){
    try {
        QString apnea_s = "";
...
QString TramaPCPAP::obtener_pinsp(){
    try {
        QString pinsp_s = "";
...
QString TramaPCPAP::obtener_tinsp(){
    try {
        QString tinsp_s = "";
...
QString TramaPCPAP::obtener_trama_ventilador(){
    try {
        QString modo = "4";
        QString t_ti = obt_ti();
        QString t_tp = obt_tp();
...
QString TramaPCPAP::obt_te(){
    try {
        QString te_s = "";
        float ti = tinsp.toFloat();
        float periodo = 60 / frecuencia.toFloat();
...
QString TramaPCPAP::obt_pwm_oxi(){
    try {
...
QString TramaPCPAP::obt_pwm_aire(){
    try {
        int temp = oxigeno.toInt();
...
QString TramaPCPAP::obt_psoporte(){
    try {
        QString ps_s = "";
        int ps_int = ps.toInt();
        if(ps_int >= 0 && ps_int < 10){
...
QString TramaPCPAP::obt_palarmax(){
    try {
        QString pmax_s = "";
