...
VentanaConfigPI::VentanaConfigPI(QWidget *parent, int width, int height, QString titulo) : QWidget(parent)
{
    try {
        ...
        label_altura = new QLabel(this);
        fuente->setPointSize(12);
        label_altura->setGeometry(QRect(offset_x + 0, offset_y + 0, 250, 30));
...
void VentanaConfigPI::setTituloColor(QColor color){
    try {
        titulo_color = color;
    } catch (std::exception &e) {
        qWarning("Error %s desde la funcion %s", e.what(), Q_FUNC_INFO );
...
void VentanaConfigPI::setMensajeColor(QColor color){
    try {
        mensaje_color = color;
    }  catch (std::exception &e) {
        qWarning("Error %s desde la funcion %s", e.what(), Q_FUNC_INFO );
...
void VentanaConfigPI::setFondoColor(QColor color){
    try {
...
void VentanaConfigPI::paintEvent(QPaintEvent* /*event*/){
    try {
       QSize s = size();
...
QColor VentanaConfigPI::colorEstado(QString estado){
    try {
        if(estado == PROCESO){
